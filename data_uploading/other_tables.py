...
def check_last_inserted():
    conn = connect_to_database()
    cur = conn.cursor()

    cur.execute("select max(id_viaje) from viajes_stm.viaje_stm")
    coso = cur.fetchone()
    print(coso)

def check_row(row):
    if (row['id_viaje'] == '' or row['con_tarjeta'] == '' or row['fecha_evento'] == '' ):
        return False
    if (row['tipo_viaje'] == '' or row['grupo_usuario'] == '' or row['cantidad_pasajeros'] == ''):
        return False
    if (row['codigo_parada_origen'] == '' or row['cod_empresa'] == '' or row['linea_codigo'] == ''):
        return False
    return True
    ...
    with open(csv_file_name, newline='') as csvfile:
        ...
        for row in reader:
            ...
            if(i<start):
                ...
                conn.commit()
                print("Commited to database, uploading to the viajes tables")
                print(f"Reading line {i}")
            else:
                ...
                (id_viaje, con_tarjeta, fecha_evento, tipo_viaje, grp_usuario, grp_usuario_esp, ordinal_tramo, cant_pasajeros,
                codigo_parada, cod_empresa, codigo_linea, sevar_codigo)
...
#csv_name = "csv_files/viajes_stm_012023.csv"
#process_data(csv_name)
import_other_tables("csv_files/viajes_stm_012023.csv")
#import_viajes_tables("csv_files/viajes_stm_012023.csv")